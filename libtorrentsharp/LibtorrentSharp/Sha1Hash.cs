...
/// <summary>
/// 20-byte SHA-1 digest. Backs the BitTorrent v1 info-hash. Allocation-free struct
/// (3 inline integer fields = 20 bytes); equality and hashing are field comparisons.
/// </summary>
public readonly struct Sha1Hash : IEquatable<Sha1Hash>
{
    /// <summary>Length in bytes of a SHA-1 digest.</summary>
    ...
    /// <summary>
    /// Builds a hash from a 20-byte buffer. Throws if the span is the wrong length.
    /// </summary>
    public Sha1Hash(ReadOnlySpan<byte> bytes)
    {
        if (bytes.Length != ByteLength)
        {
            throw new ArgumentException(
                $"Sha1Hash requires exactly {ByteLength} bytes; got {bytes.Length}.",
                nameof(bytes));
        }

        _a = BinaryPrimitives.ReadUInt64BigEndian(bytes[..8]);
        _b = BinaryPrimitives.ReadUInt64BigEndian(bytes.Slice(8, 8));
        _c = BinaryPrimitives.ReadUInt32BigEndian(bytes.Slice(16, 4));
    }

    /// <summary>Whether this hash is the all-zero sentinel libtorrent uses for "absent".</summary>
    public bool IsZero => _a == 0 && _b == 0 && _c == 0;

    /// <summary>Writes the hash into a freshly-allocated 20-byte array.</summary>
    public byte[] ToArray()
    {
    ...
    /// <summary>Writes the hash into <paramref name="destination"/> (length must be ≥ 20).</summary>
    public void WriteTo(Span<byte> destination)
    {
        ...
        BinaryPrimitives.WriteUInt32BigEndian(destination.Slice(16, 4), _c);
    }

    /// <summary>
    /// Parses a 40-char hex string. Returns false on length mismatch or non-hex chars.
    /// Case-insensitive; canonical <see cref="ToString"/> output is lowercase.
    /// </summary>
    public static bool TryParse(ReadOnlySpan<char> hex, out Sha1Hash hash)
    {
        hash = default;
