...
- **Skipped tests in v0.1.0 CI** — disabled to keep the green badge while waiting for proper fixes:
  ...
  - [ ] LibtorrentSharp test suite is `workflow_dispatch`-only in CI — the test host crashes mid-suite on the GitHub `windows-latest` runner even with Network category excluded and `parallelizeTestCollections=false`. All 288 tests pass locally. Run with `--blame-crash --blame-crash-dump-type=full` on a runner to identify the specific native-side crash, then either fix the offending test or guard it behind an env flag and re-enable the suite for push CI.
